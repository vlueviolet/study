(function (win, $, doc) {
    ...
    win.OVERWATCH.Common.ScoreLayer.prototype = {
        ...
        clickHighlight : function (e) {
            ...
            var _this = this;
            var target = $(e.currentTarget);
            _this.videoNo = target.data('videoNo').toString();
            _this.matchNo = target.data('videoNo').toString();
            console.log(_this.videoNo)
            _this.layer.css({
                display : 'none',
                opacity : 1,
                zIndex : 110
            ...
            //     console.log('commonLayer.opts.viewBefore....', this.commonScoreJSON);
            //     // if(!this.commonScoreJSON) {
            //     //     this.getDataFunc();
            //     // }
            //     // win.clearTimeout(this.viewBeforeTimeout);
            //     // this.viewBeforeTimeout = win.setTimeout($.proxy(function () {
            //     //     // this.setLayer();
            //     // }, this), 30);

                $.ajax({
                    url: '/resources/json/score_contenders.json',
                    success: function (response) {
                        _this.data = response;
                        _this.setLayer();
                        console.log(response);
                        window.commonLayer.opts.viewBefore = null;
                        window.commonLayer.layerView(_this.layer);
                    },
        ...
        setLayer : function () {
            // var scrore = (this.data) ? this.data.score : null;
            var scrore = _score;
             $.each(scrore, $.proxy(function (index, data) {
        ...
        getDataFunc : function () {
            ...
            if($('html').hasClass('division')) {
                this.commonScoreJSON = win.OVERWATCH.Common.DataCall.getData('../json/score_division.json');
            } else if($('html').hasClass('trials')) {
                this.commonScoreJSON = win.OVERWATCH.Common.DataCall.getData('../json/score_trials.json');
            } else if($('html').hasClass('contenders')) {
                this.commonScoreJSON = win.OVERWATCH.Common.DataCall.getData('../json/score_contenders.json');
            } else {
                this.commonScoreJSON = win.OVERWATCH.Common.DataCall.getData('../json/score.json');
            }
            if(!this.type) return;
            this.type = false;
            this.commonScoreJSON.done(function (json) {
                console.log('getDateFunc Done', json);
                _this.setDataFunc(json);
            });

        },
        setDataFunc : function (data) {
            console.log('setDataFunc ...', data );
                    ...
                    if(score['detail']) {
                        ...
                        $.each(score['detail'], function (index, detail) {
                            ...
                            if(detail['field_type']) {  // 무승부 셋트
                                $('.field'+ detail['field_no']).addClass('tie');
                            }
                        });
                    }
                    if(score['video_url']) {
    ...
    win.OVERWATCH.Common.DataCall = {
        getData : function (searchUrl, callback) {
            return $.ajax({
                cache : false,
                type : 'GET',
                url : searchUrl,
                ...
                success : function (data) {
                 ...
                 error: function (error) {
                    console.error('ajax call err: ' + searchUrl, error);
                    return error;
                }
            });
        }
    ...
    function commonTeamJSON(json) {
       window.commonTeamJSON = json;
    ...
    $(function () {
        ...
        if($('html').hasClass('division')) {
            var commonTeamJSON = win.OVERWATCH.Common.DataCall.getData('resources/json/team_division.json', function (data) {
                win.commonTeamJSON = data;
                $('body').trigger('page.reposition');
            });
        } else if($('html').hasClass('trials')) {
            var commonTeamJSON = win.OVERWATCH.Common.DataCall.getData('resources/json/team_trials.json', function (data) {
                win.commonTeamJSON = data;
                $('body').trigger('page.reposition');
            });
        } else if($('html').hasClass('contenders')) {
            var commonTeamJSON = win.OVERWATCH.Common.DataCall.getData('resources/json/team_contenders.json', function (data) {
                win.commonTeamJSON = data;
                $('body').trigger('page.reposition');
            });
        } else {
            var commonTeamJSON = win.OVERWATCH.Common.DataCall.getData('resources/json/team.json', function (data) {
                win.commonTeamJSON = data;
                $('body').trigger('page.reposition');
            });
        }

        win.OVERWATCH.Common.DataCall.getData('/resources/json/score_contenders.json', function (data) {
            console.log(11111);
            _score = data
        });
